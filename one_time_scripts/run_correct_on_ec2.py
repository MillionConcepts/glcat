...
def test_eclipse(eclipse):
    """ function for running aspect refine on ec2, pulling backplanes from s3 bucket
    and cleaning up afterwards. uploads aspect soln to s3 bucket. """
    ...
    with PRO.context(eclipse_str):
        ...
        asp_address = refine_eclipse(eclipse=eclipse,
                       xylist=False,
                       band="NUV",
    ...
    print("Uploading aspect solution to bucket.")
    upload_asp = f"aws s3 cp {asp_address} {bucket_path}"
    try:
        result = subprocess.run(upload_asp,
                                shell=True,
                                check=True,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE,
