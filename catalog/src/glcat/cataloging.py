...
def photometry_for_catalog(
    ...
    for ap in aperture_sizes:
        ...
        if suffix == "base":
            columns.extend(["ya_aperture_sum", "stdcolrow_aperture_sum","q_aperture_sum", "dose_col", "dose_row"])
...
def aper_alt(
    ...
    mean_col = phot['dose_col'].to_numpy()
    mean_row = phot['dose_row'].to_numpy()

    return {
        f"{band}_YA_A{aper_ix}": ya_photom,
        f"{band}_SIGDISP_A{aper_ix}": stdcolrow_photom,
        f"{band}_Q_A{aper_ix}": q_photom,
        f"{band}_DOSE_COL": mean_col,
        f"{band}_DOSE_ROW": mean_row
    }
