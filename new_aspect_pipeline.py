...
sys.path.insert(0, '/home/ubuntu/gPhoton2')
...
def refine_eclipse(
    ...
    """ main pipeline for processing an eclipse frame by frame.
     this pipeline requires that all 1s backplanes be pre-produced.
      easiest script would run backplanes for a particular eclipse at 1s
       ...
       :param backplane_root: where backplanes are
       :param aspect_root: where you want astrometry to save output
    ...
    # make aspect folder for processing
    astrom_folder_path = make_aspect_folder(aspect_root, eclipse_info)
    ...
    paths = get_base_file_paths(
        ...
        band=band,
        ext=ext,
        back_root=backplane_root,
        asp_root=aspect_root,
        mode='direct',
    ...
    base_args = [eclipse_info, backplane_root, aspect_root, xylist]
    frame_args = [
    ...
    for frame, aspect in aspects.items():
        try:
            ...
            eclipse_aspect[frame] = {
                ...
                'time': float(aspect.iloc[0]['time']),
                'frame_type': str(aspect.iloc[0]['frame_type']),
    ...
    aspect_soln_filepath = f"{aspect_root}/" \
                           f"{eclipse_info['eclipse_str']}_new_aspect.csv"
    aspect_soln.to_csv(aspect_soln_filepath)
    # clean up
...
def make_aspect_folder(aspect_root, eclipse_info):
    """ folder that astrom outputs are stored in """
    folder_path = f"{aspect_root}/e{eclipse_info['eclipse_str']}"
    import os
...
def cleanup_eclipse_files(folder_path):
...
def refine_frame(frame_series, eclipse_info, backplane_root, aspect_root, xylist):
...
def get_base_file_paths(
        ...
        back_root: Pathlike = "data",
        asp_root: Pathlike = "data",
    ...
    if legs == 0:
        paths[0] = eclipse_to_paths(eclipse=eclipse, band=band, depth=1,
                                    compression=compression, root=back_root,
                                    mode=mode, leg=legs, )
    else:
        for i in range(legs):
...
def get_backplane_filenames(
    ...
    leg_frames['aspect_output'] = aspect_root + f"/e{eclipse_info['eclipse_str']}"
